...
class splitting_preferences:
    def __init__(self, preference_breaks=[], sub_breaks=[], modify_inside=[], tag_as_text=[], kept_tags=[],
                 escape_rules=[], no_diff=[], sub_rules=None):
    ...
    # breaking rules
    def prevent_breakup(self, html, pos):
        # return an RE match for a section that should not be broken apart such
        # as <script> <style> as they can't be split like normal html
        return PROTECTED_RE.match(html, pos=pos)
        # return None

    def preference_breaks(self, html, pos):
        # return an RE match for a section that should be split this is done after white space and prevent breakup
        # but before the text and general splitting
    ...
    def sub_breaks(self, html):  # detects it text needs to be broken a second time
        for rule, threshold in self.sub_breaks_RE:
            if rule.match(html):
                return threshold
        return False
        ...
        for (rule, front, back) in self.escape_rules:
            ...
            if rule.match(text):
                if not front.match(text):
                    raise get_context("required context", front, back)
...
class html_splitter(object):
    def __init__(self, html_string, splitting_preferences=splitting_preferences()):
    ...
    def __next__(self):
        if self.end_reached:
            raise StopIteration
        while match := WS_RE.match(self.html_string,
                                   pos=self.pos):  # starting with white space (clear up all the white space we can see
            self.pos = match.end()
        if match := self.splitting_preferences.prevent_breakup(self.html_string,
                                                               self.pos):  # don't break protected sections
            self.pos = match.end()
            return match.group(0)
        if match := self.splitting_preferences.preference_breaks(self.html_string,
                                                                 self.pos):  # don't break sections that are user defined as special
            self.pos = match.end()
            return match.group(0)
...
class html_differ(SequenceMatcher):
    ...
    def set_seqs(self, a, b, protect_small_tags_a=None, protect_small_tags_b=None):
        ...
        SequenceMatcher.set_seqs(self,
                                 sum([[k] if k.startswith('<') else WORD_RE.findall(k) for k in
                                      html_splitter(a, self.splitting_preferences)], []),
                                 sum([[k] if k.startswith('<') else WORD_RE.findall(k) for k in
                                      html_splitter(b, self.splitting_preferences)], [])
                                 )
    ...
    def clean_delete(self, del_items):
        ...
        for item in del_items:
            ...
            elif item.startswith('<') and not self.splitting_preferences.treat_tag_as_text(
                    item):  # insert the text wrapped in del tags treat small tags as text
        ...
        for item in ins_items:
            ...
            if self.splitting_preferences.modify_inside(item):
                new_sub_items = sum([[k] if k.startswith('<') else WORD_RE.findall(k) for k in html_splitter(item)], [])
                    ...
                    item):  # insert the text wrapped in ins check preferences for
                if not all([WS_RE.match(i) for i in text]):  # not all white space
        ...
        if len(origin) == len(modified):
            ...
            for o, m in zip(origin, modified):
                if all([WS_RE.match(o), WS_RE.match(m)]):  # both are whitespace
                    continue
                if o.startswith('<') and m.startswith('<') and \
                        not self.splitting_preferences.treat_tag_as_text(m) and \
                        not self.splitting_preferences.treat_tag_as_text(o) and \
                        not self.splitting_preferences.modify_inside(m) and \
                        not self.splitting_preferences.modify_inside(o):
                    continue
    ...
    def diff_html(self):
        ...
        for tag, start_a, end_a, start_b, end_b in opcodes:  # main loop
            ...
            if tag == 'replace':
                ...
                #    out.write(d)
                if self.splitting_preferences.get_subrules() is not None and (
                        self.detect_sub_breaks(a[start_a:end_a]) or self.detect_sub_breaks(b[start_b:end_b])):
                    d = html_differ("".join(a[start_a:end_a]), "".join(b[start_b:end_b]),
                                    self.splitting_preferences.get_subrules())

                    lower_html = d.diff_html()
                    r = d.ratio()
                    if d.ratio() > self.splitting_preferences.sub_breaks("".join(a[start_a:end_a])) or \
                       d.ratio() > self.splitting_preferences.sub_breaks("".join(b[start_b:end_b])):
...
def git_diff(commit_a, commit_b, file_path, context=1):
    ...
    out = run(['git', '--no-pager', 'diff', '--minimal', f'--unified={context}', commit_a, commit_b, '--', file_path],
...
def process_patch(file_a, file_b, patch, splitting_preferences, min_start_a=0, min_start_b=0):
    ...
    try:
        ...
        if min_start_a < patch["start_a"] and min_start_b < patch["start_b"]:
            splitting_preferences.require_escape(text_a)
            splitting_preferences.require_escape(text_b)
        diff = html_differ(text_a, text_b, splitting_preferences=splitting_preferences)
        end = diff.diff_html()
        ...
        return process_patch(file_a, file_b, patch, splitting_preferences, min_start_a=min_start_a,
                             min_start_b=min_start_b)
...
def process_file(patch_list, file_a, file_b, split_pref):
    ...
    for i, patch in enumerate(
            patch_list):  # for each patch make and run a html diff store change requests in the replacements
        ...
        for j in range(result["patch"]["start_a"] + 1, result["patch"]["start_a"] + result["patch"]["length_a"]):
            replacements[j] = ""
...
def preference_breaks_from_json(json_dict):
    if json_dict != {}:
        end = [re.compile(f'<{tag}[^/<>]*>.*?</{tag}[^/<>]*>') for tag in
               json_dict.get("break_tags", [])] + preference_breaks_from_json(json_dict.get("sub_rules", {}))
        return end
    return []
...
def preference_sub_modify_inside_json(json_dict):
    if json_dict != {}:
        return [re.compile(f'<{tag}[^/<>]*>.*?</{tag}[^/<>]*>') for tag in
                json_dict.get("modify_inside", [])] + preference_sub_modify_inside_json(json_dict.get("sub_rules", {}))
    return []
...
def preference_text_tags_from_json(json_dict):
    if json_dict != {}:
        return [re.compile(f'<{tag}[^/<>]*>.*?</{tag}[^/<>]*>') for tag in
                json_dict.get("text_tags", [])] + preference_text_tags_from_json(json_dict.get("sub_rules", {}))
    return []


def preference_kept_tags_from_json(json_dict):
    if json_dict != {}:
        return [re.compile(f'</?{tag}[^/<>]*>') for tag in
                json_dict.get("kept_tags", [])] + preference_kept_tags_from_json(json_dict.get("sub_rules", {}))
    return []
...
def preference_no_diff_from_json(json_dict):
    if json_dict != {}:
        return [re.compile(f'<{tag}[^/<>]*>.*?</{tag}[^/<>]*>') for tag in
                json_dict.get("no_diff", [])] + preference_no_diff_from_json(json_dict.get("sub_rules", {}))
    return []
...
def preference_from_json(json_dict):
    return splitting_preferences(
        ...
        sub_rules=preference_from_json(
            json_dict["sub_rules"]) if "sub_rules" in json_dict.keys() else splitting_preferences()
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('from_commit', type=str, nargs=1, help='the old commit of the file')
    parser.add_argument('to_commit', type=str, nargs=1, help='the new commit of the file')
    parser.add_argument('file', type=str, nargs=1, help='the path to the html file to diff')
    parser.add_argument('outfile', nargs='?', type=argparse.FileType('w'), default=sys.stdout,
                        help="optional location of output file for the diffed html defaults to the console")
